...
export const StudentDashboard: React.FC = () => {
  ...
  const [studentRoll, setStudentRoll] = useState('26-PCM-01');
  const [studentClass, setStudentClass] = useState('Class 11 A (Science)');
  ...
  useEffect(() => {
    ...
    if (user.classSegment) {
      setSelectedSegment(user.classSegment);
    }
    if (user.roll) {
  ...
  return (
    <div className="py-12 md:py-16 bg-cream min-h-[calc(100vh-100px)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Compact Student Profile Details (Auto-fetched & Locked) */}
        <div className="bg-white p-5 rounded-lg border border-slate-200/50 shadow-xs mb-8 grid grid-cols-2 md:grid-cols-4 gap-6 font-sans text-left text-xs">
          <div>
            <span className="text-slate-400 font-bold uppercase block tracking-wider mb-1">STUDENT NAME</span>
            <span className="font-extrabold text-primary text-sm sm:text-base">{studentName}</span>
          </div>
          <div>
            <span className="text-slate-400 font-bold uppercase block tracking-wider mb-1">ROLL NUMBER</span>
            <span className="font-mono font-extrabold text-primary text-sm sm:text-base">{studentRoll}</span>
          </div>
          <div>
            <span className="text-slate-400 font-bold uppercase block tracking-wider mb-1">CLASS / SECTION</span>
            <span className="font-extrabold text-primary text-sm sm:text-base">{studentClass}</span>
          </div>
          <div>
            <span className="text-slate-400 font-bold uppercase block tracking-wider mb-1">LANGUAGE MEDIUM</span>
            <span className="font-extrabold text-primary text-sm sm:text-base uppercase">
              {selectedMedium === 'english' ? 'English Medium' : 'Hindi Medium (हिंदी माध्यम)'}
            </span>
          </div>
        </div>
